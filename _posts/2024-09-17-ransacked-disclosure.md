...
Our research find these vulnerabilities are present in both well-maintained open-source LTE/5G cores and in proprietary software.

## Impact

Cellular networks are considered critical infrastructure both for day-to-day communication and emergency services, to the extend that their availability and reliability is often highly [regulated](https://www.fcc.gov/network-reliability-resources) by government agencies.
...
An attacker can continuously crash the Mobility Management Entity (MME) or Access and Mobility Management Function (AMF) in an LTE/5G network, respectively, simply by sending a single small data packet over the network.
...
A number of vulnerabilities discovered cause [buffer overflows](https://en.wikipedia.org/wiki/Buffer_overflow) or similar memory corruption vulnerabilities.
...
The exploitability of memory corruption vulnerabilities widely varies; in some cases, there may be no practical way of obtaining remote access despite the presence of a buffer overflow thanks to mitigations such as Stack Canaries or ASLR. We provide a **proof-of-concept RCE exploit** for one of the vulnerabilities in SD-Core as a way of practically demonstrating the severity of these findings.

## Threat Models

These vulnerabilities fall under two threat models, the first being far more severe than the second:

1. Vulnerabilities that can be exploited by **any unauthenticated mobile device**.
...
These attacks _may_ be limited in scope to devices that are within radio distance of the LTE/5G core being attacked.
If the affected component supports Voice over WiFi (VoWiFi), however, then they can be arbitrarily exploited over the Internet--no SDR equipment required.

2. Vulnerabilities that can be exploited by an adversary who has base-station access to the cellular core.

This includes attackers that have either a) compromised a base station/femtocell, or b) gained access to the IPsec network used by base stations to communicate with the cellular network via a misconfiguration or key leak.
...
An adversary would easily be able to obtain persistent physical access to one of these devices and dump RAM/flash or carry out attacks specific to the device to gain access to its IPsec keys.
The proliferation of smaller 5G base stations in easier-to-reach locations (_not_ 100 feet in the air on a tower) also makes compromise of a regular base station a possibility.

## Vulnerability Overview

## Analysis

This section contains more detailed analysis of the causes of each vulnerability. If you are a system maintainer looking only to remediate vulnerabilities and move on, this information may not be useful to you.
...
**TODO**: used to be OOB write--different vuln?
...
Federation v1.20 were discovered to contain a buffer overflow in the
...
static status_code_e amf_as_establish_req(amf_as_establish_t* msg,
  ...
  if ((msg->nas_msg->data[1] != 0x0) && (msg->nas_msg->data[9] == 0x5c)) {
    // uninitialized access happens here
...
int convertHoAcklToProtoIe(SuccessfulOutcome_t *msg, struct proto_IE *proto_ies)
{
    ...
    memcpy(
        ...
        eRabAdmittedItem_p->dL_transportLayerAddress->size);            break;
    // ^ unchecked length of transportLayerAddress overruns struct being copied into
...
Samples:
- `AAdAgKEAAAoACEAEgLJ20AAAQAXAR8TVVwAAQAXA00ByNQAAQAXApwf9zwBCQAwgAc0/FPSAAWMm7QQAIUA/CQA+QAIcZgAjQAMOCEAAI0ACGogAI0ACHEAAI0ACACAAI0ACGIYAI0ACFiAAI0ACDIYAI0ADABCAACNAAghmAAhABIDA+5sAGkADAriXAABABcDZHHLJAEJADCAB5Y1Qz4ACFVKEiw==`

Counterexamples:
- `AAdAgKEAAAoACEAEgLJ20AAAQAXAR8TVVwAAQAXA00ByNQAAQAXApwf9zwBCQAwgAc0/FPSAAWMm7QQAIUA/CQAjQAIcZgAjQAMOCEAAI0ACGogAI0ACHEAAI0ACACAAI0ACGIYAI0ACFiAAI0ACDIYAI0ADAAGAACNAAghmAAhABIDA+5sAGkADAriXAABABcDZHHLJAEJADCAB5Y1Qz4ACFVKEiw==`
